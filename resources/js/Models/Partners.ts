export class Partners {
    ...
    private updateDataTablePartner(data, state) {
        ...
        for (const adresse of adresses) {
            adresse.innerHTML = adresse.innerHTML.replace(match, `<mark>${state.search}</mark>`)
        }
        // @ts-ignore
        for (const locality of localities) {
            locality.innerHTML = locality.innerHTML.replace(match, `<mark>${state.search}</mark>`)
        }
        // @ts-ignore
        for (const localityNum of localitiesNum) {
            localityNum.innerHTML = localityNum.innerHTML.replace(match, `<mark>${state.search}</mark>`)
        }
        this.slideInView();
