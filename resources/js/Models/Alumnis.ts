export class Alumnis {
    ...
    private updateDataTableAlumni(data, state) {
        ...
        for (const name of names) {
            name.innerHTML = name.textContent.replace(match, `<mark>${state.search}</mark>`)
        }
        // @ts-ignore
        for (const firstname of firstnames) {
            firstname.innerHTML = firstname.textContent.replace(match, `<mark>${state.search}</mark>`)
        }
        // @ts-ignore
        for (const statusElement of status) {
            let str = statusElement.textContent.replace(/<[^>]+>/g, '')
            statusElement.innerHTML = str.replace(match, `<mark>${state.search}</mark>`)
        }
        // @ts-ignore
        for (const begin of begins) {
            begin.innerHTML = begin.textContent.replace(match, `<mark>${state.search}</mark>`)
        }
        // @ts-ignore
        for (const end of ends) {
            end.innerHTML = end.textContent.replace(match, `<mark>${state.search}</mark>`)
        }
