...
<main id="content">
        ...
        <div
            ...
            <div class="mb-7 col-span-5 max-w-full">
                <section aria-labelledby="description">
                    <h3 class="font-sans text-lg xl:text-2xl md:text-xl font-medium mb-5" id="description"
                        aria-level="2"
                        role="heading">{{__('job.job_subtitle')}}</h3>
                    <div class="flex flex-col gap-6 text-lg leading-8 xl:leading-10 xl:text-xl">
                       {!!$job->description!!}
                    </div>
                    <div class="mt-8">
    ...
    <article class="bg-yellow-600 2xl:px-48 xl:px-32 px-10 pb-36 pt-20" aria-labelledby="other">
        <div class="flex flex-col md:flex-row justify-between mb-8">
            <h2 class="xl:text-4xl md:text-3xl text-2xl uppercase font-extrabold font-sans mb-4 xl:mb-20" id="other"
                aria-level="2"
        ...
        <div class="lg:grid lg:grid-cols-2 xl:gap-x-24 lg:gap-x-12 xl:gap-y-8 flex flex-col gap-y-4">
            @foreach($jobs as $job_ref)
                <article aria-labelledby="{{$job_ref->translation->where('locale',app()->getLocale())->first()->slug}}"
                         class="bg-white-100 py-6 px-8 rounded-3xl relative xl:text-lg">
                    <h3 id="{{$job_ref->translation->where('locale',app()->getLocale())->first()->slug}}" aria-level="3"
                        role="heading"
                        class="font-medium text-xl xl:text-2xl mb-3 font-medium">{{$job_ref->translation->where('locale',app()->getLocale())->first()->name}}</h3>
                    {!! $job_ref->translation->where('locale',app()->getLocale())->first()->excerpt !!}
                    <a class="linkcard"
    ...
    <article class="xl:px-32 2xl:px-48 px-10 mt-36 mb-36" aria-labelledby="alumnis">
        <div class="flex flex-col md:flex-row justify-between mb-6">
            <h2 class="xl:text-4xl md:text-3xl md:max-w-md md:leading-10 text-2xl uppercase font-bold text-yellow-800 mb-6 lg:max-w-full xl:mb-16 font-sans"
                id="alumnis"
                aria-level="2"
