...
<article
        aria-labelledby="{{$doc_ref->slug}}" {{ $attributes->class(['group relative bg-yellow-100 py-6 px-8 rounded-3xl']) }}>
    <div class="flex justify-between">
        <div>
            <h4 id="{{$doc_ref->slug}}" aria-level="4" role="heading"
                class="font-medium text-lg xl:text-2xl mb-2">{{$doc_ref->name}}</h4>
        </div>
    </div>
    <p class="mb-4 xl:mb-10 xl:text-lg">{{$doc_ref->excerpt}}</p>
...
{{--           href="{{$doc_ref->link}}">{{__('')}} {{$course->name}}</a>--}}
