...
<article {{ $attributes->class(['bg-yellow-600 xl:px-30 lg:px-16 2xl:px-48 px-10 py-20 mt-20 xl:mt-80']) }}>
    ...
    <div class="xl:grid xl:grid-cols-2 xl:gap-x-24 xl:gap-y-8 flex flex-col gap-y-4">
        @foreach($testimonials as $testimonial_ref)
            <x-testimonial_article :testimonial_ref="$testimonial_ref->translation->where('locale',app()->getLocale())->first()"/>
        @endforeach
    </div>
</article>
