...
@endphp
<article {{ $attributes->class(['md:slide-in group relative hover:bg-orange-100 py-6 px-8 rounded-3xl']) }}
         aria-labelledby="{{$book->slug}}">
    <div class="flex justify-between">
        <div>
            <h4 id="{{$book->slug}}" aria-level="4" role="heading"
                class="font-medium md:text-xl text-lg xl:text-2xl 2xl:text-3xl mb-2">{{$book->name}}</h4>
        </div>
    </div>
