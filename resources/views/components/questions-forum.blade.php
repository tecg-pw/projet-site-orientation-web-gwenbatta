                    ...
                    <div class="order-2 flex-1 flex flex-col xl:ml-4 justify-center gap-y-2 xl:gap-4">
                        <div class="flex flex-col  xl:gap-4">
                            ...
                            <div class="flex relative justify-between xl:justify-between">
                                <p class="xl:text-base sm:text-sm text-base uppercase self-end ">{{$subject->user->firstname}} {{$subject->user->name}}</p>
                                <p class="xl:text-base sm:text-sm self-end">{{$subject->created_at->format('d M. Y')}}</p>
                            </div>
                        </div>
                        <div class="order-3 flex flex-row justify-between">
                            <div class="flex gap-4 items-center">
                                <p class="xl:text-base text-sm uppercase font-medium bg-orange-500/40 mr-4 px-3 xl:px-6 pb-1 pt-1.5 rounded-lg text-green-700">
                                    {{ucwords($subject->tag->translation->where('locale', app()->getLocale())->first()->name)}}</p>
                                <p class="xl:text-lg sm:text-base">{{__('forum.number_comment').$subject->comments_count}}</p>
                            </div>
                            ...
                            <div class="order-2 flex-1 flex flex-col xl:ml-4 justify-center gap-y-2 xl:gap-4">
                                <div class="flex-col flex xl:gap-4">
                                    ...
                                    <div class="flex relative justify-between">
                                        <p class="sm:text-sm xl:text-base uppercase self-end">{{$latest->user->firstname}} {{$latest->user->name}} </p>
                                        <p class="sm:text-sm xl:text-base self-end">{{$latest->created_at->format('d M. Y')}}</p>
                                    </div>
                                </div>
                                <div class="order-3 flex justify-between">
                                    <div class="flex gap-4">
                                        <p class=" font-medium bg-orange-500/40 mr-4 px-3 xl:px-6 pb-1 pt-1.5 rounded-lg text-sm xl:text-base uppercase text-green-700">
                        ...
                        <article class="hover:bg-orange-100 group bg-white-100 relative flex p-2 sm:p-4 xl:p-6 rounded-xl"
                            ...
                            <div class="order-2 flex-1 flex flex-col xl:ml-4 justify-center gap-y-2 xl:gap-4">
                                <div class="flex-col flex xl:gap-4">
                                    ...
                                    <div class="flex relative justify-between">
                                        <p class="sm:text-sm xl:text-base uppercase self-end">{{$rating->user->firstname}} {{$rating->user->name}} </p>
                                        <p class="sm:text-sm xl:text-base self-end">{{$rating->created_at->format('d M. Y')}}</p>
                                    </div>
                                </div>
                                <div class="order-3 flex justify-between">
                                    <div class="flex gap-4">
