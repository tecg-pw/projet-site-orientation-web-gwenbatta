...
@if($teacher_ref->isTeacher)
    <article {{ $attributes->class(['even:mt-36 relative group']) }} aria-labelledby="{{$teacher_ref->slug}}">
        ...
        <div class="relative bg-yellow-100 rounded-xl px-4 py-8 left-10">
            <h3 class="font-medium text-2xl mb-5" id="{{$teacher_ref->slug}}" aria-level="3" role="heading">
            ...
            <a class="hover:text-orange-500 text-xl underline text-green-700"
               href="mailto:{{$teacher_ref->mail}}">{{$teacher_ref->mail}}</a>
            <div class="flex {{$teacher_ref->status === 'teachalumni'? 'gap-40': 'gap-60'}} items-center mt-4 ">
                ...
                @else
                    <p class="uppercase xl:text-lg">{{$teacher_ref->status}}</p>
                @endif
            </div>
        </div>
    </article>
