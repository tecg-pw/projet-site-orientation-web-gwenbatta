...
@foreach($comments as $comment)
    <article {{ $attributes->class(['slide-in mb-7 flex flex-col bg-yellow-100 py-3 px-4 xl:py-6 xl:px-10 rounded-xl']) }}
        ...
        <div class="wysiwyg search-comment first-letter:uppercase xl:leading-8 xl:mt-6 mt-4">
            {!! $comment->content !!}
        </div>
        ...
        <div class="relative flex order-7 gap-1.5">
            <form action="/{{app()->getLocale()}}/forum/{{$comment->subject->slug}}/comment/like/{{$comment->id}}"
                  method="post">
                @csrf
