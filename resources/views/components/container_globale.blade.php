...
<div  {{ $attributes }}>
                            ...
                            <div class="order-2 flex-1 flex flex-col xl:ml-4 justify-center gap-y-2 xl:gap-4">
                                <div class="flex flex-col  xl:gap-4">
                                    <div class="flex gap-2">
                                        <h4 class="order-2 font-medium text-lg xl:text-xl"
                                            id="{{$subject->slug}}"
                                            role="heading"
                                            aria-level="4">{{$subject->subject}}</h4>
                                        @if($subject->resolved)
                ...
                <div class="md:grid xl:grid-cols-3 md:grid-cols-2 flex sm:items-center flex-col gap-8 justify-center">
                    @foreach($results['projects'] as $project)
                        <x-project_search :person="$project->person->translation->where('locale', app()->getLocale())->first()"
                                   :project_ref="$project->translation->where('locale',app()->getLocale())->first()"></x-project_search>
                    @endforeach
                </div>
            </article>
        @endif
    ...
    @elseif(empty($results))
        <div>
            <p class="text-lg font-medium xl:text-2xl 2xl:text-3xl mb-7">{{__('search.no_result')}}</p>
            <p class="text-lg font-medium xl:text-2xl mb-1.5 2xl:text-3xl">{{__('search.help_search')}}</p>
            <ul>
                @foreach(__('search.list_help_search') as $key=>$help)
                    <li class="flex items-center gap-3">
                        <svg xmlns="http://www.w3.org/2000/svg" width="7" height="7" viewBox="0 0 7 7">
                            <circle cx="3.5" cy="3.5" r="3.5"
                                    fill="#6f704d"/>
                        </svg>
                        <p class="text-lg xl:text-xl 2xl:text-2xl">{{__($help)}}</p>
                    </li>
                @endforeach
            </ul>
