            ...
            <div class="absolute z-30 bg-green-200 w-full h-full flex items-center justify-center">
                <div class="flex px-10 py-10 items-center flex-col rounded-lg bg-white-100">
                    <form class="flex items-center flex-col" action="/{{app()->getLocale()}}/forum/{{$subject->slug}}/comment/destroy/{{Request::query('delete-comment')}}"
                          method="post">
                        @csrf
                        <label class="text-green-500 text-lg font-medium mb-4 " for="delete">{{__('forum.verify_delete_label_comment')}}</label>
                        <div class="flex gap-x-10">
                            ...
                            <input class="text-white-100 bg-red-600 px-6 border-2 border-red-600 hover:bg-white-100 hover:text-red-600 py-2 rounded-lg text-xl font-sans font-semibold" name="delete" id="delete" type="submit" value="{{__('forum.value__delete')}}">
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="absolute z-30 bg-green-200 w-full h-full flex items-center justify-center">
                <div class="flex px-10 py-10 items-center flex-col rounded-lg bg-white-100">
                    <form class="flex items-center flex-col" action="/{{app()->getLocale()}}/forum/subject/destroy/{{Request::query('delete-subject')}}" method="post">
                        @csrf
                        <label class="text-green-500 text-lg font-medium mb-4" for="delete">{{__('forum.verify_delete_label')}}</label>
                        <div class="flex gap-x-10">
                            <a class="hover:text-orange-500 font-sans self-center text-green-700 text-xl font-semibold underline "
                               href="/{{app()->getLocale()}}/forum/{{$subject->slug}}#sujet">{{__('forum.button_back')}}</a>
                            <input class="text-white-100 bg-red-600 px-6 border-2 border-red-600 hover:bg-white-100 hover:text-red-600 py-2 rounded-lg text-xl font-sans font-semibold" name="delete" id="delete" type="submit" value="{{__('forum.value__delete')}}">
                            ...
                            @if($subject->user->id === auth()->id())
                                <div class="flex justify-end pr-4">
                                    <a href="/{{app()->getLocale()}}/forum/question/modify/{{$subject->slug}}">
                                        ...
                                        <span><svg class="h-8 w-8 text-green-700"  width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>  <path d="M4 20h4l10.5 -10.5a1.5 1.5 0 0 0 -4 -4l-10.5 10.5v4" />  <line x1="13.5" y1="6.5" x2="17.5" y2="10.5" /></svg></span>
                                    </a>
                                    <a href="?delete-subject={{$subject->id}}#sujet">
                                        <span class="sr-only">{{__('forum.value__delete')}}</span>
                                        <span><svg class="h-8 w-8 text-red-500"  width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>  <line x1="4" y1="7" x2="20" y2="7" />  <line x1="10" y1="11" x2="10" y2="17" />  <line x1="14" y1="11" x2="14" y2="17" />  <path d="M5 7l1 12a2 2 0 0 0 2 2h8a2 2 0 0 0 2 -2l1 -12" />  <path d="M9 7v-3a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v3" /></svg></span>
                                    </a>
                                </div>
                            @endif
                        </div>
                    ...
                    @if(request()->has('add-comment'))
                        <div class="flex flex-col bg-yellow-100 py-6 px-10 rounded-xl">
                            <form action="/{{app()->getLocale()}}/forum/{{$subject->slug}}" method="post"
                                ...
                                <input type="submit" value="{{__('forum.value__add')}}"
                                       class="text-white-100 bg-green-700 px-6 mt-4 border-2 border-green-700 hover:bg-white-100 hover:text-green-700 py-2 rounded-lg text-xl font-sans font-semibold">
                            </form>
                        </div>
                    @endif
                            ...
                            <div id="{{$comment->slug}}"
                                ...
                                @if(\Request::query('edit-comment')===$comment->slug)
                                    <div class="flex flex-col bg-yellow-100 rounded-xl mt-6">
                                        <form action="/{{app()->getLocale()}}/forum/{{$subject->slug}}/comment/{{$comment->id}}" method="post"
                                              class="relative">
                                            @csrf
                                            <a class="hover:text-orange-500 absolute -top-6 right-1 font-sans text-end text-green-700 text-xl font-semibold underline px-4 py-6 self-end"
                                               href="/{{app()->getLocale()}}/forum/{{$subject->slug}}#comment">{{__('forum.button_back')}}</a>
                                            ...
                                            <input type="submit" value="{{__('forum.value__modify')}}"
                                                   class="text-white-100 bg-green-700 px-6 mt-4 border-2 border-green-700 hover:bg-white-100 hover:text-green-700 py-2 rounded-lg text-xl font-sans font-semibold">
                                        </form>
                                    </div>
                                @else
