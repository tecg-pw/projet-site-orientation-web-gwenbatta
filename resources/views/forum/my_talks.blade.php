...
<main id="content">
    ...
    <section class="mt-20" aria-labelledby="forum">
        ...
        <x-sort_search.sort_by_forum id="sortmysanswers" :urlAction="$urlAction" :status="$status" :tags="$tags" :created="$created"
        ...
        <div class="flex items-start md:flex-row flex-col xl:gap-32 mb-12 px-10 2xl:px-48 xl:px-32 lg:px-16">
            ...
            <a class="2xl:text-2xl xl:text-xl text-lg hover:text-orange-500 text-center underline text-green-700 font-semibold rounded-lg bg-orange-100 font-sans px-5 py-2"
               href="/{{app()->getLocale()}}/forum/my-talks#forum">{{__('forum.my_talks')}}</a>
        </div>
        <div class="lg:grid lg:grid-cols-5 mb-36">
            <x-mytalks_paginated class="lg:col-span-3 flex flex-col xl:pr-14 lg:pr-7 2xl:pl-48 xl:px-32 lg:px-16 px-10" :comments="$comments"/>
            <x-aside_forum id="mytalks" :urlAction="$urlAction" :latests="$latests" :ratings="$ratings"></x-aside_forum>
        </div>
    </section>
</main>
