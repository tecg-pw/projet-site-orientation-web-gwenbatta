...
<main id="content">
    ...
    <section class="px-10 xl:px-36 mt-14 mb-36 gap-24 items-center" aria-labelledby="profs">
        ...
        <div class="xl:grid xl:grid-cols-2 flex flex-col gap-y-60 xl:mt-60 xl:gap-y-40 xl:gap-x-60">
            @foreach($teachers as $teacher_ref)
                <article class="even:mt-36 relative group" aria-labelledby="{{$teacher_ref->translation->slug}}">
                    <img class="rounded-lg absolute -z-10 -top-[70%] group-even:-top-[120%]" src="{{$teacher_ref->translation->avatar}}"
                         alt="avatar">
                    <div class="relative bg-yellow-100 rounded-xl px-4 py-8 left-10">
                        <h3 class="font-medium text-2xl mb-5" id="{{$teacher_ref->translation->slug}}" aria-level="3" role="heading">
                            <a class="hover:text-orange-500" href="/{{str_replace('_','-',app()->getLocale())}}/bottin/{{$teacher_ref->translation->status === 'teachalumni'? 'teachalumni':'teacher'}}/{{$teacher_ref->translation->slug}}">{{$teacher_ref->translation->firstname}} {{$teacher_ref->translation->name}}</a>
                        </h3>
                        <a class="hover:text-orange-500 text-xl underline text-green-700" href="mailto:{{$teacher_ref->translation->mail}}">{{$teacher_ref->translation->mail}}</a>
                        <div class="flex gap-60 items-center mt-4 ">
                            <div class="flex gap-3">
                                <div class="relative group">
                                    ...
                                    <a class="linkcard" href="{{$teacher_ref->translation->link_github}}">Aller sur github de {{$teacher_ref->translation->name}}</a>
                                </div>
                                <div class="relative group">
                                    <svg class="group-hover:scale-110" xmlns="http://www.w3.org/2000/svg" width="25"
                                    ...
                                    <a class="linkcard" href="{{$teacher_ref->translation->instagram}}">Aller sur instagram de {{$teacher_ref->translation->name}}</a>
                                </div>
                                <div class="group relative">
                                    ...
                                    <a class="linkcard" href="{{$teacher_ref->translation->linkedin}}">Aller sur le
                                        linkedin de {{$teacher_ref->translation->name}}</a>
                                </div>
                            </div>
                            <p class="text-green-500 text-lg items-end uppercase">{{$teacher_ref->translation->status}}</p>
                        </div>
                    </div>
                </article>
            @endforeach
    ...
    <article class="bg-yellow-600 px-10 xl:px-36 pb-36 pt-20" aria-labelledby="jobs">
        ...
        @foreach($jobs as $job_ref)
            <article aria-labelledby="{{$job_ref->translation->slug}}" class="bg-white-100 py-6 px-8 rounded-3xl relative">
                <h3 id="{{$job_ref->translation->slug}}" aria-level="3" role="heading"
                    class="font-medium text-xl xl:text-2xl mb-3 font-medium">{{$job_ref->translation->name}}</h3>
                <p class="xl:text-lg">{{$job_ref->translation->excerpt}}</p>
                <a class="linkcard"
                   href="/{{str_replace('_','-',app()->getLocale())}}/about/job/{{$job_ref->translation->slug}}">{{__('about.job_more'). $job_ref->translation->slug}}</a>
            </article>
        @endforeach
        </div>
    </article>
