...
<main id="content">
    <div class="flex items-center px-10 2xl:px-48 xl:px-32 lg:px-16 mt-20">
        ...
        <a href="/{{app()->getLocale()}}/bottin/teachalumni/{{$teachalumni->slug}}" class="underline hover:text-orange-500 text-green-500 mt-0.5 text-lg leading-8 xl:leading-10 xl:text-xl 2xl:text-2xl">
            {{$teachalumni->firstname}} {{$teachalumni->name}}
        </a>
    </div>
    <section class="mt-20 px-10 2xl:px-48 xl:px-32 lg:px-16 flex flex-col mb-20 xl:mb-36 gap-x-11" aria-labelledby="name">
        <div class=" flex flex-col justify-between text-green-700 font-sans font-semibold md:flex-row">
            <h2 id="name" role="heading" aria-level="2"
                class="xl:text-4xl 2xl:text-5xl md:text-3xl text-2xl uppercase font-extrabold text-yellow-800 font-sans">{{$teachalumni->firstname}} {{$teachalumni->name}}</h2>
        ...
        <div class="flex flex-col md:flex-row gap-6 xl:gap-12 mt-16">
            <div class="mt-4 mb-7 order-2">
                <div class="flex gap-12">
                    <div>
                        <div class="mb-4 flex gap-8 md:flex-col md:gap-y-2 xl:flex-row xl:justify-between uppercase text-lg xl:text-xl 2xl:text-2xl">
                            <p class="uppercase">{{__('people.teachalumni_status')}}</p>
                            <p>{{$teachalumni->begin->format('Y')}}</p>
                            <a class="hover:text-orange-500 underline text-green-700"
                               href="/{{app()->getLocale()}}/about/job/single">{{$teachalumni->job}}</a>
                        </div>
                        <div class="flex gap-y-1.5 flex-col xl:gap-y-2.5 mb-5 text-lg xl:text-xl 2xl:text-2xl">
                            ...
                            <a class="hover:text-orange-500 text-green-700 underline text-xl"
                               href="">{{$teachalumni->link_portfolio}}</a>
                        </div>
                        <div class="flex flex-1 mb-6 gap-5">
                            <div class="relative group">
                                ...
                                <a class="linkcard" href="{{$teachalumni->link_github}}"></a>
                            </div>
                            <div class="group relative">
                                ...
                                <a class="linkcard" href="{{$teachalumni->instagram}}"></a>
                            </div>
                            <div class="group relative">
                                <svg class="group-hover:scale-110" xmlns="http://www.w3.org/2000/svg" width="30"
                                ...
                                <a class="linkcard" href="{{$teachalumni->linkedin}}"></a>
                            </div>
                        </div>
                    </div>
                </div>
                @if(strip_tags($teachalumni->description) === "")
                    <p class="flex flex-col gap-6 leading-8  text-lg xl:text-xl 2xl:text-2xl xl:leading-10">
            ...
            <div class="xl:min-w-[345px] md:min-w-[250px] flex flex-col">
                <picture>
                    @if($teachalumni->srcset && $teachalumni->srcset['full'])
                        @foreach($teachalumni->srcset['full'] as $size => $path)
                            <source media="(max-width: {{$size}}px)"
                                    srcset="/{{$path}}">
                        @endforeach
                    @endif
                    <img
                        src="{{$teachalumni->logos && $teachalumni->logos['full'] ? '/' . $teachalumni->logos['full'] : '/'.$teachalumni->logo}}"
    ...
    <article class="bg-yellow-600 2xl:px-48 xl:px-32 lg:px-16 px-10 pb-24 pt-20" aria-labelledby="classes">
        <div class="flex xl:flex-row flex-col justify-between mb-8">
            <h2 id="classes" role="heading" aria-level="2" class="xl:text-4xl 2xl:text-5xl md:text-3xl text-2xl uppercase font-extrabold font-sans mb-4 xl:mb-20">{{__('people.bottin_course') . ( $teachalumni->firstname . ' ' .$teachalumni->name)}}</h2>
            <a class="hover:text-orange-500 text-green-700 underline font-sans md:text-xl xl:text-2xl font-semibold 2xl:text-3xl" href="/{{app()->getLocale()}}/about#course">{{__('people.teacher_course')}}</a>
        </div>
    ...
    <article class="px-10 2xl:px-48 xl:px-32 lg:px-16 pb-40 pt-20" aria-labelledby="projects">
        <div class="justify-between flex flex-col mb-8 md:flex-row">
            <h2 id="projects" role="heading" aria-level="2"
                class="xl:text-4xl 2xl:text-5xl text-yellow-800 md:text-3xl text-2xl uppercase font-extrabold font-sans mb-4 xl:mb-20">{{__('project.project_other')}}</h2>
        ...
        <div class="md:grid md:grid-cols-2 xl:grid-cols-3 xl:gap-8 flex flex-col gap-y-4 {{count($projects) !== 0 ? 'items-center' : ''}} justify-center">
            @foreach($projects as $project_ref)
                <x-project :person="$teachalumni" :project_ref="$project_ref->translation->where('locale',app()->getLocale())->first()"></x-project>
            @endforeach
            @if(count($projects) === 0)
                <p class="xl:text-xl text-lg 2xl:text-2xl" >{{__('people.bottin_no_project')}}</p>
            @endif
        </div>
    </article>
</main>
