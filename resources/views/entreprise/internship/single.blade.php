    ...
    <section class="xl:px-36 px-10 mt-20 mb-20 xl:mb-40" aria-labelledby="{{$offer_ref->slug}}">
        <div class="flex xl:flex-row flex-col justify-between mb-8">
            <h2 id="{{$offer_ref->slug}}" role="heading" aria-level="2"
                class="xl:text-4xl text-2xl text-yellow-800 uppercase font-extrabold font-sans mb-4 xl:mb-20">{{$offer_ref->name}}</h2>
        ...
        <div class="flex xl:flex-row flex-col">
            <div class="flex flex-col gap-16">
                <article class="even:mt-36 relative mt-48" aria-labelledby="{{$offer->partner->translation->where('locale',app()->getLocale())->first()->slug}}">
                    <img class="rounded-lg absolute -z-10 bottom-2/3" src="{{$offer->partner->translation->where('locale',app()->getLocale())->first()->logo}}"
                         alt="avatar">
                    <div class="relative bg-yellow-100 rounded-xl px-4 py-8 left-10">
                        <h3 id="{{$offer->partner->translation->where('locale',app()->getLocale())->first()->slug}}" role="heading" aria-level="3" class="underline font-medium text-xl xl:text-2xl mb-2">
                            <a class="text-green-700 hover:text-orange-500" href="/{{str_replace('_','-',app()->getLocale())}}/entreprise/partner/{{$offer->partner->translation->where('locale',app()->getLocale())->first()->slug}}">
            ...
            @foreach($offers as $other_offer)
                <article class="group hover:bg-orange-100 relative flex flex-col bg-white-100 py-4 px-4 xl:py-6 xl:px-8 rounded-3xl"
                    ...
                    <div class="flex flex-1 items-center">
                        <div class="flex-1 order-2 ml-4">
                            <div class="xl:flex-row flex-col flex justify-between">
                                <h3 id="{{$other_offer->translation->where('locale',app()->getLocale())->first()->slug}}" role="heading" aria-level="4" class="text-lg xl:text-2xl">{{$other_offer->translation->where('locale',app()->getLocale())->first()->name}}</h3>
                    ...
                    <a class="linkcard underline text-green-700 font-sans font-semibold self-end"
                       href="/{{str_replace('_','-',app()->getLocale())}}/entreprise/internship/{{$other_offer->translation->where('locale',app()->getLocale())->first()->slug}}">{{__('En savoir plus' . $other_offer->translation->where('locale',app()->getLocale())->first()->slug)}}</a>
                </article>
            @endforeach
        </div>
    </article>
