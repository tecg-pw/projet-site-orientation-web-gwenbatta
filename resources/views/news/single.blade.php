...
<main id="content">
    <section aria-labelledby="{{$new->slug}}">
        <div class="flex md:flex-row flex-col px-10 xl:px-36 2xl:px-48 justify-between mt-20">
        ...
        <div class="xl:px-36 2xl:px-48 px-10 flex gap-20 md:text-lg xl:text-xl mb-5">
            @if($new->date !== null)
                <p>{{ucwords($new->date->translatedFormat('d F Y '))}}</p>
            @endif
            <p>{{$new->lieu}}</p>
        </div>
        <div class="xl:px-36 2xl:px-48 px-10 xl:grid xl:grid-cols-10 items-center mb-36 gap-x-20">
            ...
            <div class="sr-only xl:not-sr-only col-span-5">
